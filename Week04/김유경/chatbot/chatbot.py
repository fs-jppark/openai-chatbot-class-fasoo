...
def get_text_chunks(text) -> list[Document]:
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=250, chunk_overlap=50, length_function=tiktoken_len)
    return text_splitter.split_documents(text)

# 쿼리된 참고문서와 질문으로 프롬프트를 만들어 보세요.
def get_prompt_refer_doc(docs: dict, query: str):
    ref_doc = [item["documents"] for item in docs]
    return f"참고 문서를 참고하여 질문에 답하세요.\n참고문서 : {ref_doc}, 질문 : {query}"
